...
const generateTransaction = (id, config) => {
  const date = new Date(
    config.dateRange.start.getTime() + Math.random() * (config.dateRange.end.getTime() - config.dateRange.start.getTime()),
  );
  ...
  const cityTax = config.fees.cityTax ? randomBetween(config.fees.cityTax.min, config.fees.cityTax.max) : undefined;
  const countyTax = config.fees.countyTax ? randomBetween(config.fees.countyTax.min, config.fees.countyTax.max) : undefined;
  const serviceFee = randomBetween(config.fees.serviceFee.min, config.fees.serviceFee.max);
  const paymentGatewayFee = randomBetween(config.fees.paymentGatewayFee.min, config.fees.paymentGatewayFee.max);
  ...
  return {
    ...
    cellphone:
      Math.random() > 0.3
        ? `555${Math.floor(Math.random() * 10000000)
            .toString()
            .padStart(7, "0")}`
        : undefined,
...
fs.writeFile(
  "/Users/levisherman/Documents/coding-projects/projects/parking-dashboard/frontend/src/data/transactions.json",
