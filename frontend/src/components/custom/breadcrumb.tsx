...
const Breadcrumb = React.forwardRef<HTMLElement, BreadcrumbProps>(({ className, children, separator, ...props }, ref) => {
...
const BreadcrumbItem = React.forwardRef<HTMLLIElement, BreadcrumbItemProps>(({ className, children, ...props }, ref) => {
  const { separator, isLastChild, ...rest } = props as InternalBreadcrumbItemProps;
  ...
  if (!isInsideBreadcrumb) {
    throw new Error(`${BreadcrumbItem.displayName} must be used within ${Breadcrumb.displayName}.`);
  }
...
const getValidChildren = (children: React.ReactNode) =>
  React.Children.toArray(children).filter((child) => {
    ...
    throw new Error(`${Breadcrumb.displayName} can only have ${BreadcrumbItem.displayName} as children.`);
