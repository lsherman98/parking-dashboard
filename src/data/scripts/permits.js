...
export const generatePermits = (count, dateRange) => {
  return Array.from({ length: count }, (_, index) => {
    const name = generateRandomName();
    const startDate = new Date(dateRange.min.getTime() + Math.random() * (dateRange.max.getTime() - dateRange.min.getTime()));
...
// Usage example:
const permits = generatePermits(100, { min: new Date(2023, 0, 1), max: new Date(2023, 11, 31) });
fs.writeFile(
  "/Users/levisherman/Documents/coding-projects/projects/parking-dashboard/frontend/src/data/permits.json",
