  ...
  const transactions = database.transactions
    ...
    .filter((t) => {
      if (params.statuses.length > 0) {
        return params.statuses.includes(t.status);
      }
      return true;
    })
    .filter((t) => {
    ...
    setTimeout(() => {
      const data = {
        ...
        stats: {
          ...
          violationRevenue: transactions.filter((t) => t.status === "violation").reduce((acc, t) => acc + t.total, 0),
          enforcementCommission: transactions.filter((t) => t.status === "violation").reduce((acc, t) => acc + t.total, 0) * 0.15,
          processingFees: transactions.reduce((acc, t) => acc + t.payment_gateway_fee, 0),
          transactionCountChange: 26,
          violationsCountChange: 14,
          violationRevenueChange: 10,
          enforcementCommissionChange: 10,
          processingFeesChange: -5,
        },
      };
      resolve(data);
    }, 200);
